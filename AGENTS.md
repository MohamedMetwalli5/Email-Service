...
- Backend tests: `@WebMvcTest` (controllers, sliced, with Mockito) and `@DataJpaTest` (repository slices on H2). Slice tests use H2 + `TestSecurityConfig` and the `jwt()` MockMvc post-processor to populate a principal without real JWKS. Slices do not load the full context, so populate collaborators via `@MockBean`. Integration tests (`AuthFlowIT`/`MailFlowIT`/`NotificationFlowIT`) run via failsafe on `mvn verify`.
...
- Auth in `MailFlowIT` and `NotificationFlowIT` uses the same `jwt()` MockMvc post-processor as the controller slices, not real auth-service tokens. This keeps each IT scoped to its own container lifecycle; resource-server filter behavior (401 on missing/invalid token, subject extraction) is already proven by the `@WebMvcTest` slices. An end-to-end IT exercising real JWKS fetch from a live auth-service container is a documented upgrade path, not a current gap.
- `MailFlowIT` covers the full mailbox lifecycle (send -> inbox + outbox -> move-to-trash -> trashbox -> permanent delete) and asserts exactly one `EmailSentEvent` on `email.sent` (trash and delete publish nothing). `NotificationFlowIT` publishes the same `event_id` twice and requires the feed to stay at size 1 for a stability window before asserting - this is intentional, not a sloppy poll loop. Do not "simplify" it back to a single sample or a broken dedup regression can slip through after the test returns.
